...
function setupConfigHandlers(ipcMain2, configStore2) {
  ipcMain2.handle("get-custom-css", async () => {
    return configStore2.getCustomCSS();
  });
  ipcMain2.handle("set-custom-css", async (_event, css) => {
    configStore2.setCustomCSS(css);
    return true;
  });
}
...
ipcMain$1.handle("reload-file", async () => {
  ...
  try {
    ...
    const stats = fs2.statSync(currentFilePath);
    const MAX_FILE_SIZE = 10 * 1024 * 1024;
    if (stats.size > MAX_FILE_SIZE) {
      throw new Error("ファイルサイズが大きすぎます（最大10MB）");
    }
    const content2 = await readFile(currentFilePath, "utf-8");
    return { filePath: currentFilePath, content: content2 };
  } catch (error2) {
    console.error("File reload error:", error2);
    throw error2;
  }
});
